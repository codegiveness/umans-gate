...
test("allows first N requests, rejects N+1", () => {
  ...
  expect(rl.check(1000).allowed).toBe(true);
  expect(rl.check(2000).allowed).toBe(true);
  expect(rl.check(3000).allowed).toBe(true);
  const r = rl.check(4000);
