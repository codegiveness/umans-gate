// Unit tests for src/vision/detect.ts — intent-aware ImagePart extraction.
//
// Validates the four intent fields on ImagePart:
...
// --- cheapImageSignal / inferExtFromUrl / shouldRewrite ---

describe("cheapImageSignal", () => {
  test("true for body containing image_url type", () => {
    expect(
      cheapImageSignal('{"type":"image_url","image_url":{"url":"data:image/png;base64,abc"}}'),
    ).toBe(true);
  });
  test("true for body containing image type with source", () => {
    expect(
      cheapImageSignal('{"type":"image","source":{"type":"base64","media_type":"image/png"}}'),
    ).toBe(true);
  });
  test("false for plain text body", () => {
    expect(cheapImageSignal('{"messages":[{"role":"user","content":"hello"}]}')).toBe(false);
  });
  test("false for empty string", () => {
    expect(cheapImageSignal("")).toBe(false);
  });
});

describe("inferExtFromUrl", () => {
  test("extracts png media type", () => {
    expect(inferExtFromUrl("https://example.com/cat.png")).toBe("image/png");
  });
  test("extracts jpeg media type (normalizes jpg -> image/jpeg)", () => {
    expect(inferExtFromUrl("https://example.com/cat.jpg")).toBe("image/jpeg");
    expect(inferExtFromUrl("https://example.com/dog.jpeg")).toBe("image/jpeg");
  });
  test("extracts tiff media type (normalizes tif -> image/tiff)", () => {
    expect(inferExtFromUrl("https://example.com/scan.tif")).toBe("image/tiff");
    expect(inferExtFromUrl("https://example.com/scan.tiff")).toBe("image/tiff");
  });
  test("returns null for no extension", () => {
    expect(inferExtFromUrl("https://example.com/cat")).toBeNull();
  });
  test("returns null for non-image extension", () => {
    expect(inferExtFromUrl("https://example.com/doc.pdf")).toBeNull();
  });
});

describe("shouldRewrite", () => {
  test("never strategy -> false always", () => {
    expect(shouldRewrite("never", null, "openai")).toBe(false);
    expect(shouldRewrite("never", true, "anthropic")).toBe(false);
    expect(shouldRewrite("never", false, "openai")).toBe(false);
  });
  test("always strategy -> true always", () => {
    expect(shouldRewrite("always", null, "openai")).toBe(true);
    expect(shouldRewrite("always", true, "anthropic")).toBe(true);
    expect(shouldRewrite("always", false, "openai")).toBe(true);
  });
  test("catalog + null supports -> false", () => {
    expect(shouldRewrite("catalog", null, "openai")).toBe(false);
  });
  test("catalog + false supports -> true (rewrite needed)", () => {
    expect(shouldRewrite("catalog", false, "openai")).toBe(true);
    expect(shouldRewrite("catalog", false, "anthropic")).toBe(true);
  });
  test("catalog + via-handoff + openai -> true", () => {
    expect(shouldRewrite("catalog", "via-handoff", "openai")).toBe(true);
  });
  test("catalog + via-handoff + anthropic -> false", () => {
    expect(shouldRewrite("catalog", "via-handoff", "anthropic")).toBe(false);
  });
  test("catalog + true supports + forceInterceptCapable=false -> false", () => {
    expect(shouldRewrite("catalog", true, "openai", false)).toBe(false);
  });
  test("catalog + true supports + forceInterceptCapable=true -> true", () => {
    expect(shouldRewrite("catalog", true, "openai", true)).toBe(true);
  });
});
