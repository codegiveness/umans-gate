// Integration tests for the proxy handler's streaming/SSE response path and
// capture behavior. Runs the proxy in-process for direct DB access (no
// subprocess, no dashboard HTTP roundtrip for capture inspection).
...
/** Flush the write-behind queue then read the latest capture for a path. */
async function getLatestCapture(path: string): Promise<CaptureRow | null> {
  await proxy.queue.flushNow();
  const list = proxy.db.list(50);
  const cap = list.find((c) => c.path === path);
  if (!cap) return null;
  return proxy.db.get(cap.id);
}
