// Task 9: Config additions for intent-aware vision.
// Verifies the 7 new config keys load with correct defaults, are hot-reloadable
// (NOT in RESTART_REQUIRED_FIELDS), and apply to the live ProxyConfig on reload.
...
describe("Task 9: vision intent config defaults", () => {
...
describe("Task 9: vision intent config hot-reload", () => {
  test("all 7 keys are hot-reloadable (not restart-required)", () => {
    ...
    // Every new key must be classified as applied (hot-reloadable),
  ...
  test("reload applies all 7 new values to the live ProxyConfig", () => {
    ...
    applyReloadToConfig(live, fresh, oldRaw, newRaw);
