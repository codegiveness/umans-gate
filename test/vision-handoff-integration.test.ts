...
describe("Vision handoff integration", () => {
  ...
  // catalog strategy requires a mock upstream that also serves /v1/models and
  // /v1/models/info so ModelsClient learns umans-glm-5.2 has supports_vision=false.
  ...
  test("12. catalog + non-vision model → image rewritten before forward (Bug 1)", async () => {
    const upstream = startMockLlmUpstreamWithCatalog();
    const vision = startMockVisionUpstream({ description: "A red pixel on white background." });
    const proxy = await startProxy({
      TARGET: `http://127.0.0.1:${upstream.port}`,
      STAMP_CLAUDE_CODE_ENABLED: "false",
      WARMER_ENABLED: "false",
    ...
    try {
      // Wait for ModelsClient to fetch /v1/models + /v1/models/info.
      for (let i = 0; i < 50; i++) {
  ...
  test("13. catalog + non-vision model → vision call request_body persisted with intent (Bug 2)", async () => {
    const upstream = startMockLlmUpstreamWithCatalog();
    const vision = startMockVisionUpstream({ description: "A red pixel." });
    const proxy = await startProxy({
      TARGET: `http://127.0.0.1:${upstream.port}`,
  ...
  test("14. catalog + vision-capable model → image passthrough, no vision call", async () => {
    const upstream = startMockLlmUpstreamWithCatalog();
    const vision = startMockVisionUpstream({ description: "Should not be called." });
    const proxy = await startProxy({
      TARGET: `http://127.0.0.1:${upstream.port}`,
      STAMP_CLAUDE_CODE_ENABLED: "false",
      WARMER_ENABLED: "false",
    ...
    try {
      ...
      // umans-flash is vision-capable in the mock catalog — shouldRewrite
      // returns false → image passes through untouched.
      const sentBody = {
        model: "umans-flash",
        max_tokens: 50,
        messages: [
