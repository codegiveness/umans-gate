...
as a structured array; the data model was ready for navigation, but
...
5. VersionSection renders only on the General pane. It is version /
   one-click-update detail and pairs with Server/Credentials (General
   fields). Persisting it on Experimental/Advanced eats vertical space
   where it is irrelevant. The global `UpdateIndicator`
