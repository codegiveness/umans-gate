...
- Rate limiter: upstream snapshot gate compares `requests_in_window`
  (unweighted, from `/v1/usage`) against `requestsHardCap −
  request_rate_margin` (default 50) and rejects locally before
  forwarding when the wallet is near its cap; local burst fallback
  (off by default via `never_limit_requests`) for inter-poll smoothing,
  rejects with `503` when the gate fires
- Live gate stats broadcast over WebSocket (`type: "gate"`, request
  usage included)
