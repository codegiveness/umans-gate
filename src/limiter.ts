...
class CircuitBreaker {
  ...
  record429(type: "concurrency" | "rate_limit" | "gateway"): void {
    ...
    if (this.concurrency429s.length >= this.threshold && this.state === "closed") {
      this.state = "open";
      this.openedAt = now;
    }
  }
...
class Semaphore {
  ...
  setHardCap(newHardCap: number): boolean {
    ...
    if (this.active > cap) {
      ...
      log.warn(
        `[limiter.ts] setHardCap(${newHardCap}) reduced cap below active=${this.active / SCALE}; ${excess} permits remain in-flight until completion (transient over-cap — see setHardCap docs)`,
      );
    }
    this.assertInvariants();
    return true;
  ...
  private assertInvariants(): void {
    if (!(SCALE <= this.limit && this.limit <= this.hardCap)) {
      log.warn(
        `[limiter.ts] invariant violation: SCALE(${SCALE}) <= limit(${this.limit}) <= hardCap(${this.hardCap})`,
      );
    }
